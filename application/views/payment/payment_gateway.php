...
$tap = json_decode(get_settings('tap'));
				...
				<div class="col-md-3">
					...
					<div class="row payment-gateway stripe" onclick="selectedPaymentGateway('stripe')">
						<div class="col-12">
							...
							<img class="payment-gateway-icon" height="35" src="<?php echo base_url('assets/payment/stripe.png'); ?>">
						</div>
					</div>
				<?php endif; ?>

					<?php if ($tap[0]->active != 0) : ?>
					<div class="row payment-gateway tap" onclick="selectedPaymentGateway('tap')">
						<div class="col-12">
							<img class="tick-icon paypal-icon" src="<?php echo base_url('assets/payment/tick.png'); ?>">
							<img class="payment-gateway-icon" src="<?php echo base_url('assets/payment/tap.png'); ?>">
						</div>
					</div>
					<?php endif; ?>
			...
			<div class="col-md-8">
				<div class="w-100">
					...
					<div class="w-100 float-left">
						...
						<form action="<?php echo site_url('home/tap_checkout'); ?>" method="post" class="tap-form form">
							<hr class="border mb-4">
							<input type="hidden" name="total_price_of_checking_out" value="<?php echo $total_price_of_checking_out; ?>">
							<button type="submit" class="payment-button float-right"><?php echo get_phrase('pay_by_tap'); ?></button>
						</form>
...
function selectedPaymentGateway(gateway) {
	if (gateway == 'paypal') {
		...
		$(".tap").css("border", "2px solid #00D04F");
		$('.tap-icon').show();
		$('.tap-form').show();
	}
}
