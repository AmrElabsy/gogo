...
class Crud_model extends CI_Model
{
	...
	public function update_tap_settings()
	{
		// update paypal keys
		$tap_info = array();
		$tap['active'] = $this->input->post('paypal_active');
		$tap['mode'] = $this->input->post('paypal_mode');
		$tap['sandbox_client_id'] = $this->input->post('sandbox_client_id');
		$tap['sandbox_secret_key'] = $this->input->post('sandbox_secret_key');

		$tap['production_client_id'] = $this->input->post('production_client_id');
		$tap['production_secret_key'] = $this->input->post('production_secret_key');

		array_push($tap_info, $tap);

		$data['value']    =   json_encode($tap_info);
		$this->db->where('key', 'tap');
		$this->db->update('settings', $data);

		$data['value'] = html_escape($this->input->post('tap_currency'));
		$this->db->where('key', 'tap_currency');
		$this->db->update('settings', $data);
	}

	public function tap_checkout($data) {
		...
		$config = array(
			...
			'Password' => 'f2c15@q8',
			'MerchantID' => '11943952',
			'ErrorURL' => "localhost/paymentss/index.php",
			'PaymentOption' => 'ALL', // optional. default (ALL)
			'AutoReturn' => 'N', // optional. default (Y)
			'CurrencyCode' =>'SAR', // optional. default (KWD)
		...
		$merchant =
			array(
				// Used when payment is done, to store data in database
				'ReturnURL' => "localhost/paymentss/index.php",
				'ReferenceID' => uniqid(),
			);
		$products =
			array(
